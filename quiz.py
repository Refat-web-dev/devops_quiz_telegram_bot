...
sample_questions = [
    ...
    {
        "question": "Какая комбинация короткой формы записи отвечает за данные права?",
        "answers": [
            "Все права для всех: 777",
            "Все права для владельца, на чтение и исполнение для остальных: 755",
            "Чтение и запись для владельца, группы и остальных: 666"
        ],
        "correct_answer": "Примечание: Чтение — 4, запись — 2, исполнение — 1. Несколько прав складываются."
    },
    {
    ...
    {"question": "Какой командой вывести 5 последних строк файла?", "answers": ["a) cat file --lines=5", "b) head file lines=5", "c) tail --lines=5 file", "d) tail file --lines=5"], "correct_answer": "c"},
    ...
    {"question": "Какие executors бывают у gitlab-runner?", "answers": ["a) docker", "b) shell", "c) ssh", "d) gitlab-executor"], "correct_answer": "d"},
    {"question": "За что отвечает SSH?", "answers": ["a) Передача информации", "b) Управление компьютером", "c) UDP или TCP соединение", "d) TCP-туннелирование"], "correct_answer": "b"},
    ...
    {"question": "Выбрать уровни OSI", "answers": ["a) физический", "b) сетевой", "c) сеансовый", "d) виртуальный", "e) межсетевой", "f) технический", "g) транспортный", "h) представления", "i) прикладной"], "correct_answer": "a, b, c, g, h, i"},
    ...
    {"question": "Как посмотреть данные по сетевому интерфейсу?", "answers": ["a) tcpdump -i eth0", "b) tcpdump eth0", "c) tcpdump --interface eth0", "d) tcpdump -l eth0"], "correct_answer": "a, c"},
    ...
    {"question": "Что выведет следующий код:", "answers": [
        "a) true", 
        "b) 0", 
        "c) 1", 
        "d) $"], "correct_answer": "d"
    ...
    {"question": "Какой командой создаётся user?", "answers": ["a) добавить в файл sudoers", "b) использовать команду adduser", "c) использовать команду useradd", "d) добавить в файл /etc/passwd"], "correct_answer": "b"},
    ...
    {"question": "Что будет выведено?", "answers": ["a) false", "b) echo $", "c) 1", "d) true"], "correct_answer": "b"},
...
# Начать квиз
async def start_kwiz(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Инициализация квиза
    active_quizzes[chat_id] = {"current_index": 0, "scores": {}}
...
async def send_question(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not quiz:
        await update.message.reply_text("Квиз не запущен. Используйте /start_kwiz, чтобы начать.")
        return

    question_data = sample_questions[quiz["current_index"]]
...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if answer not in ["a", "b", "c", "d", "e"]:
        return  # Игнорируем сообщения не из набора [a, b, c, d]
    ...
    if quiz["current_index"] < len(sample_questions) - 1:
...
async def show_records(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    leaderboard = "\n".join(
        [f"{record['username']}: {record['score']}" for record in sorted(global_leaderboard.values(), key=lambda x: x['score'], reverse=True)]
    )
...
# Основной код
def main():
    # Замени 'YOUR_TOKEN_HERE' на токен, полученный от BotFather
